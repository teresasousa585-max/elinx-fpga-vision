...
module gray_morphology #(
  ...
  // 辅助函数 max5：比较 5 个灰度样本并返回最大值，供膨胀运算的邻域归约使用。
  function [7:0] max5;
    // 接口信号：input 接收上游数据/控制，output 返回处理结果/状态，inout 连接双向器件总线。
    input [7:0] a, b, c, d, e;
    // 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
  ...
  // 辅助函数 min5：比较 5 个灰度样本并返回最小值，供腐蚀运算的邻域归约使用。
  function [7:0] min5;
    // 接口信号：input 接收上游数据/控制，output 返回处理结果/状态，inout 连接双向器件总线。
    input [7:0] a, b, c, d, e;
    // 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
