...
// -----------------------------------------------------------------------------
// 正文导读：实例化算法支路，根据主模式和子模式选择输出，并保证像素数据与 HS/VS/DE 同步。
...
module image_process_pipe (
  ...
  // 子模块例化 1（rgb2ycbcr）：以流水线方式完成 RGB 到 YCbCr 的定点转换，同时延迟原始 RGB 旁路以保持时序一致。
  ...
  wire [23:0] hsv_data;
  // 子模块例化 2（rgb2hsv）：完成 RGB 到 HSV 的定点转换，为肤色区域识别等基于色调和饱和度的处理提供数据。
  rgb2hsv u_rgb2hsv (
      .clk(clk_hdmi),
      .rst(sys_rst),
      .i_hs(raw_hs),
  ...
  wire [7:0] equ_Y;
  // 子模块例化 3（histogram_equalization）：统计灰度直方图并生成均衡化映射，提高低对比度画面的灰度分布范围。
  ...
  wire [23:0] hdr_rgb;
  // 子模块例化 4（hdr_tone_mapping_color）：根据亮度分量执行定点色调映射，并保持彩色信息，实现硬件 HDR 动态范围压缩。
  ...
  wire [23:0] bil_rgb;
  // 子模块例化 5（bilateral_filtering_proc_to_hdmi）：组织颜色转换、邻域缓存、双边核计算和时序对齐，输出可直接显示的滤波视频流。
  ...
  wire [7:0] morph_Y;
  // 子模块例化 6（gray_morphology）：在灰度 3×3 邻域上按子模式执行腐蚀或膨胀，用于目标区域去噪与结构增强。
  gray_morphology #(
      .H_DISP(1024)
  ) u_morphology (
      .clk(clk_hdmi),
  ...
  // 子模块例化 7（mean_filter_3x3）：对 RGB 3×3 邻域执行均值滤波，用于图像放大后的平滑和锯齿抑制。
  ...
  // 1. 实例化 3x3 行缓存窗口
  // 子模块例化 8（sobel_line_buffer）：缓存三行灰度像素并输出 3×3 邻域，同时对齐窗口有效信号与行场同步。
  sobel_line_buffer #(
      .H_TOTAL(11'd1344)
  ) u_sobel_lb (
      .i_clk      (clk_hdmi),
  ...
  // 2. 实例化 Sobel 计算模块
  // 子模块例化 9（sobel_calc）：计算 Sobel 水平与垂直梯度，依据阈值生成二值边缘图。
  ...
  // 时序过程 2：由 clk_hdmi posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge clk_hdmi) begin
