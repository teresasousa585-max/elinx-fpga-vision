...
module top (
  ...
  wire rx_done;
  wire [7:0] rx_data;
  // [Ethereal注释] 子模块例化 3（uart_rx）：按设定波特率完成 UART 起始位检测、数据采样和字节有效指示。
  uart_rx #(
      .CLK_FRE(100),
      .BAUD_RATE(1000000),
      .DATA_WIDTH(8)
  ...
  wire [7:0] target_sub;
  wire mode_valid;
  // [Ethereal注释] 子模块例化 4（uart_cmd_parser）：识别 0xAA/0x55 帧界定符，解析主模式与子模式并产生更新脉冲。
  ...
  // [Ethereal注释] 子模块例化 5（cdc_handshake）：采用请求/应答握手在异步时钟域间可靠传递多位模式控制数据。
  cdc_handshake #(
      .DATA_WIDTH(12)
  ) u_cdc_hdmi (
      .rst(sys_rst),
  ...
  // [Ethereal注释] 子模块例化 6（cdc_handshake）：采用请求/应答握手在异步时钟域间可靠传递多位模式控制数据。
  cdc_handshake #(
      .DATA_WIDTH(12)
  ) u_cdc_pclk (
      .rst(sys_rst),
  ...
  // [Ethereal注释] 时序过程 2：由 clk_hdmi posedge，sys_rst posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // [Ethereal注释] 时序过程 3：由 CAM_PCLK posedge，sys_rst posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // [Ethereal注释] 子模块例化 7（sii9134_i2c_init）：通过 I2C 初始化 SII9134 HDMI 发送器寄存器。
  sii9134_i2c_init #(
      .CLK_FRE(100)
  ) u_hdmi_i2c (
      .i_clk(clk_sys),
  ...
  // [Ethereal注释] 时序过程 4：由 clk_sys posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge clk_sys) begin
    if (sys_rst) begin
      cam_pwr_cnt <= 24'd0;
      r_cam_pwdn <= 1'b1;
  ...
  // [Ethereal注释] 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
  reg [23:0] pclk_cnt = 0;
  // [Ethereal注释] 时序过程 5：由 CAM_PCLK posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // [Ethereal注释] 子模块例化 8（ov5640_i2c_init）：通过 I2C 寄存器序列初始化 OV5640 的分辨率、像素格式和输出时序。
  ov5640_i2c_init #(
      .CLK_FRE(100)
  ) u_cam_i2c (
      .i_clk(clk_sys),
  ...
  // [Ethereal注释] 子模块例化 9（ov5640_capture）：在像素时钟域采集 OV5640 RGB565 数据及同步信号，形成内部视频流。
  ...
  // [Ethereal注释] 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
  reg href_d1, href_d2;
  // [Ethereal注释] 时序过程 6：由 CAM_PCLK posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  reg [10:0] cam_x;
  reg [9:0] cam_y;
  // [Ethereal注释] 时序过程 7：由 CAM_PCLK posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge CAM_PCLK) begin
    if (cam_frame_vsync) begin
      cam_x <= 0;
      cam_y <= 0;
  ...
  // [Ethereal注释] 时序过程 8：由 CAM_PCLK posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // [Ethereal注释] 子模块例化 10（sdram_top）：封装 SDRAM 控制器和读写 FIFO，提供视频帧缓存的统一顶层接口。
  ...
  // [Ethereal注释] 组合连线组 1：从 sdram_rd_req 开始的连续赋值随右值立即更新，不增加寄存器延迟。
  assign sdram_rd_req =
  ...
  // [Ethereal注释] 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
  wire [15:0] ram_q;
  // [Ethereal注释] 子模块例化 11（line_buffer）：封装片上存储器 IP，为行缓存、帧内缓存或直方图统计提供存储资源。
  ...
  // [Ethereal注释] 子模块例化 12（video_rotator_bram）：利用双口 BRAM 缓存和地址重映射实现 90° 旋转与仿射错切，并对齐帧控制信号。
  ...
  // [Ethereal注释] 时序过程 9：由 clk_hdmi posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  wire [23:0] raw_rgb;
  // [Ethereal注释] 子模块例化 13（hdmi）：生成显示扫描时序，从帧缓存读取像素并形成 RGB888、HS、VS、DE 基准视频流。
  hdmi u_hdmi (
      .i_pclk(clk_hdmi),
      .i_rst(sys_rst),
      .i_rgb(rgb888_in),
  ...
  // 纯净图像管线独立管理
  // [Ethereal注释] 子模块例化 14（image_process_pipe）：实例化引导滤波/磨皮和暗光增强支路，根据主模式 10、11 选择最终输出视频流。
  ...
  // [Ethereal注释] 子模块例化 15（phy_reg_config）：通过 MDIO 配置 PHY 工作模式，并向系统报告初始化完成状态。
  ...
  // [Ethereal注释] 时序过程 10：由 clk_hdmi posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge clk_hdmi) begin
    if (sys_rst) begin
      algo_vs_d1 <= 1'b0;
      algo_vs_d2 <= 1'b0;
  ...
  reg        safe_fifo_aclr;
  // [Ethereal注释] 时序过程 11：由 clk_hdmi posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge clk_hdmi) begin
    if (sys_rst) begin
      safe_rst_cnt   <= 5'd0;
      safe_fifo_aclr <= 1'b1;
  ...
  // [Ethereal注释] 时序过程 12：由 clk_125m posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // [Ethereal注释] 子模块例化 16（fifo_async_16b）：封装 FIFO IP，在数据通路中完成缓存、速率匹配或跨时钟域传输。
  ...
  // [Ethereal注释] 时序过程 13：由 clk_125m posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge clk_125m) begin
  ...
  // [Ethereal注释] 组合连线组 1：从 eth_16b_rdreq 开始的连续赋值随右值立即更新，不增加寄存器延迟。
  assign eth_16b_rdreq = eth_16b_rdreq_reg;

  // [Ethereal注释] 时序过程 14：由 clk_125m posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge clk_125m) begin
    if (sys_rst || eth_frame_vsync) begin
      rdreq_d1 <= 1'b0;
      eth_fifo_wrreq <= 1'b0;
  ...
  // [Ethereal注释] 子模块例化 17（UDP_Send）：生成包含视频载荷的以太网/IPv4/UDP 数据帧，并按帧起点协议发送 RGB565 分片。
  UDP_Send u_udp_send (
      .Clk        (clk_125m),
      .Rst_n      (!sys_rst),
