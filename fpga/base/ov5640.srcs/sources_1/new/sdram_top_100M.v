...
// =============================================================================
// -----------------------------------------------------------------------------
// 正文导读：封装 100 MHz SDRAM 控制器与 FIFO，提供兼容的视频帧缓存接口。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：读写地址、突发长度、FIFO 清空和跨时钟握手必须保持一致，避免帧错位或数据溢出。
// -----------------------------------------------------------------------------
// 模块 sdram_top_100M：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module sdram_top_100M (
  ...
  // 组合连线组 1：从 O_sdram_dqm 开始的连续赋值随右值立即更新，不增加寄存器延迟。
  assign O_sdram_dqm = 2'b00;
  assign O_sdram_clk = I_out_clk;

  //sdram_fifo_ctrl
  // 子模块例化 1（sdram_fifo_ctrl）：协调异步读写 FIFO、突发地址和帧边界，连接视频时钟域与 SDRAM 时钟域。
  ...
  // sdram_control
  // 子模块例化 2（sdram_control_100M）：实现 100 MHz 工作时钟下的 SDRAM 初始化、刷新和突发读写时序。
