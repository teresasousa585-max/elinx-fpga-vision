...
// 作者：Ethereal
// 工程分区：基础图像处理工程（base）
// 文件名称：sdram_control_100M.v
// 主要模块：sdram_control_100M
// 功能分类：帧缓存控制
// 功能说明：实现 100 MHz 工作时钟下的 SDRAM 初始化、刷新和突发读写时序。
// 输入概述：读写时钟、FIFO 请求、帧地址、突发长度及待写像素数据。
// 输出概述：读出像素数据、SDRAM 命令/地址/数据和初始化完成状态。
// 时序约束：跨越视频与 SDRAM 时钟域；FIFO 清空、帧边界和突发握手必须保持同步。
// 关联文件：sdram_param.v、sdram_top_100M.v
// 维护要求：修改端口、位宽、流水线延迟或模式编码时，必须同步更新上层例化与项目文档。
// =============================================================================
`timescale 1ns / 1ps
//主要优化点：结合了多个模块综合一起，并且采用多个技术来提高SDRAM的带宽
// -----------------------------------------------------------------------------
// [Ethereal注释] 正文导读：实现 100 MHz 工作时钟下的 SDRAM 初始化、刷新和突发读写时序。
// [Ethereal注释] 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// [Ethereal注释] 修改约束：读写地址、突发长度、FIFO 清空和跨时钟握手必须保持一致，避免帧错位或数据溢出。
// -----------------------------------------------------------------------------
// [Ethereal注释] 模块 sdram_control_100M：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module sdram_control_100M (
  ...
  // 时序参数配置 (100MHz 下的稳定参数，单位为时钟周期，1周期=10ns)
  // [Ethereal注释] 参数配置：综合期确定位宽、容量、频率或算法强度，修改后需重新验证资源和时序。
  localparam TRP = 3;  // 原24ns -> 需2.4拍，向上取整保持为 3 
  localparam TRC = 7;  // 原64ns -> 需6.4拍，向上取整为 7
  localparam TRSC = 3;  // 模式寄存器配置等待，保持 3 拍不变
  localparam TRCD = 3;  // 原24ns -> 需2.4拍，向上取整保持为 3
  localparam TCL  = 3;    // CAS潜伏期，保持 3 (注意：千万别改，否则需要同步修改底层初始化指令)
  localparam TWR = 2;  // 原16ns -> 需1.6拍，向上取整保持为 2
  localparam T_REF = 751;  // 原7808ns -> 需780拍，设为 750 留出安全刷新余量
