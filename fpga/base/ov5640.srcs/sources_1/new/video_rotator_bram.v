...
// -----------------------------------------------------------------------------
// 正文导读：利用双口 BRAM 缓存和地址重映射实现 90° 旋转与仿射错切，并对齐帧控制信号。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：读写地址、突发长度、FIFO 清空和跨时钟握手必须保持一致，避免帧错位或数据溢出。
// -----------------------------------------------------------------------------
// 模块 video_rotator_bram：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module video_rotator_bram (
  ...
  // 时序过程 1：由 wr_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // 时序过程 2：由 rd_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // 时序过程 3：由 rd_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge rd_clk) begin
    if (rst_rd) begin
      rot_mode_d1 <= 2'd0;
      rot_mode_d2 <= 2'd0;
  ...
  reg r_vsync, r_de;
  reg [15:0] r_rgb;
  // 时序过程 4：由 wr_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge wr_clk) begin
    r_vsync <= i_vsync;
    r_de <= i_de;
    r_rgb <= i_rgb;
  ...
  // 时序过程 5：由 wr_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge wr_clk) begin
    if (rst_wr) begin
      vsync_d1 <= 1'b0;
      vsync_d2 <= 1'b0;
  ...
  // 时序过程 6：由 rd_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge rd_clk) begin
    if (rst_rd) begin
  ...
  // 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
  reg pingpong_rd;
  // 时序过程 7：由 rd_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge rd_clk) begin
  ...
  // 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
  reg [16:0] wr_offset;
  // 时序过程 8：由 wr_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge wr_clk) begin
  ...
  // 时序过程 9：由 wr_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge wr_clk) begin
  ...
  // 时序过程 10：由 rd_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge rd_clk) begin
  ...
  // 时序过程 11：由 rd_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge rd_clk) begin
  ...
  // 子模块例化 1（ram_cascaded_128k）：级联片上 RAM，为旋转与帧内地址重排提供大容量像素缓存。
  ...
  // 时序过程 12：由 rd_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge rd_clk) begin
    if (rst_rd) begin
      rd_en_s3 <= 1'b0;
      rd_en_s4 <= 1'b0;
  ...
  // 组合连线组 1：从 o_de 开始的连续赋值随右值立即更新，不增加寄存器延迟。
