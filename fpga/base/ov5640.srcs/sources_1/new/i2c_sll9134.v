...
// -----------------------------------------------------------------------------
// 正文导读：通过 I2C 初始化 SII9134 HDMI 发送器寄存器。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：协议字段、有效脉冲和跨时钟控制必须成组更新，并与上位机及外设时序保持一致。
// -----------------------------------------------------------------------------
// 模块 sii9134_i2c_init：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module sii9134_i2c_init #(
  ...
  always @(*) begin
    // 分支选择 1：依据 lut_index 选择状态或算法路径；default 覆盖非法或空闲条件。
    case (lut_index)
  ...
  // 组合连线组 1：从 io_i2c_scl 开始的连续赋值随右值立即更新，不增加寄存器延迟。
  ...
  always @(posedge i_clk) begin
    if (i_rst) begin
      ...
      // 分支选择 2：依据 state 选择状态或算法路径；default 覆盖非法或空闲条件。
      case (state)
        S_IDLE: begin
          r_scl <= 1'b1;
          r_sda <= 1'b1;
        ...
        S_START: begin
          // 分支选择 3：依据 step 选择状态或算法路径；default 覆盖非法或空闲条件。
          case (step)
            0: begin
              r_scl <= 1'b1;
              r_sda <= 1'b1;
        ...
        S_SEND_BYTE: begin
          // 分支选择 4：依据 step 选择状态或算法路径；default 覆盖非法或空闲条件。
          case (step)
            0: begin
              r_scl <= 1'b0;
        ...
        S_ACK: begin
          // 分支选择 5：依据 step 选择状态或算法路径；default 覆盖非法或空闲条件。
          case (step)
            0: begin
              r_scl <= 1'b0;
              r_sda <= 1'b1;
        ...
        // 正常的完成一次寄存器写入
        S_STOP: begin
          // 分支选择 6：依据 step 选择状态或算法路径；default 覆盖非法或空闲条件。
          case (step)
            0: begin
              r_scl <= 1'b0;
              r_sda <= 1'b0;
        ...
        // 报错后的总线安全释放
        S_STOP_ERR: begin
          // 分支选择 7：依据 step 选择状态或算法路径；default 覆盖非法或空闲条件。
          case (step)
            0: begin
              r_scl <= 1'b0;
              r_sda <= 1'b0;
