...
// -----------------------------------------------------------------------------
// [Ethereal注释] 正文导读：级联片上 RAM，为旋转与帧内地址重排提供大容量像素缓存。
// [Ethereal注释] 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// [Ethereal注释] 修改约束：读写地址、突发长度、FIFO 清空和跨时钟握手必须保持一致，避免帧错位或数据溢出。
// -----------------------------------------------------------------------------
// [Ethereal注释] 模块 ram_cascaded_128k：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module ram_cascaded_128k (
  ...
  // [Ethereal注释] 生成逻辑：依据综合期参数展开重复硬件结构，不产生运行时软件循环。
  generate
    // 告诉综合器：帮我把下面的电路复制 2 份
    for (i = 0; i < 2; i = i + 1) begin : ram_blocks
      // 实例化底层的 64K RAM IP 核
      // [Ethereal注释] 子模块例化 1（ram_64k_ip）：封装片上存储器 IP，为行缓存、帧内缓存或直方图统计提供存储资源。
      ram_64k_ip u_ram (
          .rdclock  (rd_clk),
          .wrclock  (wr_clk),
  ...
  // [Ethereal注释] 时序过程 1：由 rd_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // [Ethereal注释] 组合连线组 1：从 q 开始的连续赋值随右值立即更新，不增加寄存器延迟。
