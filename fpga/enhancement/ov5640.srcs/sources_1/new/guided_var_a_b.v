...
module guided_var_a_b #(
    ...
    wire [23:0] mean_y_sq;
    // 子模块例化 1（lpmmult_12_12）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 子模块例化 2（rom_reciprocal_guided）：封装只读存储器 IP，提供算法查找表或定点运算常量。
    ...
    // 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
    wire [41:0] a_mult;
    // 子模块例化 3（lpmmult_24_18）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 子模块例化 4（lpmmult_12_12）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 子模块例化 5（shift_delay_64w_8d）：封装移位寄存器 IP，为像素、系数或同步信号提供固定拍数延迟。
    ...
    // 组合连线组 1：从 o_a_val 开始的连续赋值随右值立即更新，不增加寄存器延迟。
