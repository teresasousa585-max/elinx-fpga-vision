...
module box_filter_ab(
    ...
    // 子模块例化 1（lpmmult_18_13）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 子模块例化 2（lpmmult_18_13）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 取高位作为定点数还原
    // 组合连线组 1：从 o_mean_a 开始的连续赋值随右值立即更新，不增加寄存器延迟。
    ...
    // 按照拼装顺序解包输出
    // 组合连线组 1：从 o_hs_sync 开始的连续赋值随右值立即更新，不增加寄存器延迟。
