...
// -----------------------------------------------------------------------------
// 正文导读：生成显示扫描时序，从帧缓存读取像素并形成 RGB888、HS、VS、DE 基准视频流。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：像素数据、有效信号和 HS/VS 必须保持同拍；跨时钟数据必须使用 FIFO 或握手。
// -----------------------------------------------------------------------------
// 模块 hdmi：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module hdmi (
  ...
  // 组合连线组 1：从 o_fifo_rd_req 开始的连续赋值随右值立即更新，不增加寄存器延迟。
  assign o_fifo_rd_req = pre_de;

  // 消除毛刺的 VSYNC 边沿检测 (用于外部帧同步/清空 FIFO)
  // 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
  reg vs_d1, vs_d2;
  // 时序过程 2：由 i_pclk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge i_pclk) begin
    if (i_rst) begin
      vs_d1 <= 1'b0;
      vs_d2 <= 1'b0;
  ...
  // 时序过程 3：由 i_pclk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge i_pclk) begin
    if (i_rst) begin
      r_hs      <= 1'b0;  // 正极性默认拉低
      r_vs      <= 1'b0;
