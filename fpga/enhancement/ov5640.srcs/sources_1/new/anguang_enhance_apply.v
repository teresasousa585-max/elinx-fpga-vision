...
`timescale 1 ns/ 1 ps
// -----------------------------------------------------------------------------
// 正文导读：根据估计照度计算像素增益，完成暗部提升、亮部保护与输出限幅。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：像素数据、有效信号和 HS/VS 必须保持同拍；跨时钟数据必须使用 FIFO 或握手。
// -----------------------------------------------------------------------------
// 模块 anguang_enhance_apply：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module anguang_enhance_apply(
    ...
    // 子模块例化 1（anguang_gain）：封装暗光增强增益计算 IP，根据照度参数生成像素增益。
    ...
    // 子模块例化 2（lpmmult_12_8）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 子模块例化 3（lpmmult_12_8）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 子模块例化 4（lpmmult_12_8）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 子模块例化 5（lpmmult_12_8）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    //乘法器打两拍
// 子模块例化 6（lpmmult_13_13_signed）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
...
// 子模块例化 7（lpmmult_13_13_signed）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
...
// 子模块例化 8（lpmmult_13_13_signed）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
lpmmult_13_13_signed u_b_comp_mult (
    ...
    // 赋值输出端，对应顺延后的寄存器
    // 组合连线组 1：从 o_rgb_final 开始的连续赋值随右值立即更新，不增加寄存器延迟。
