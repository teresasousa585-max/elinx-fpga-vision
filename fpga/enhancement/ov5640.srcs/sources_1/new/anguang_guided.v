...
// 子模块例化 1（guided_line_buffer）：构造引导图的局部窗口，输出均值与方差计算所需的邻域数据。
guided_line_buffer #(
   .H_TOTAL(H_TOTAL) 
...
// 子模块例化 2（box_filter_y）：对引导图窗口执行盒式滤波，得到局部亮度均值及相关统计量。
...
// 子模块例化 3（guided_var_a_b）：根据局部统计量计算引导滤波线性系数 a、b。
guided_var_a_b #(
...
// 子模块例化 4（guided_line_buffer_a_b）：缓存系数 a、b 并构造其局部窗口，为系数均值计算提供数据。
...
// 子模块例化 5（box_filter_ab）：对线性系数 a、b 执行盒式均值滤波，并对齐中心像素旁路。
...
// 子模块例化 6（guided_final_rebuild）：使用均值系数和中心引导值重建滤波输出 q=mean(a)×I+mean(b)。
...
// 组合连线组 1：从 o_hs_out 开始的连续赋值随右值立即更新，不增加寄存器延迟。
