...
// -----------------------------------------------------------------------------
// 正文导读：融合原始像素与双边平滑结果，抑制皮肤纹理噪声并保留主要边缘。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：像素数据、有效信号和 HS/VS 必须保持同拍；跨时钟数据必须使用 FIFO 或握手。
// -----------------------------------------------------------------------------
// 模块 meiyan_rebuild：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module meiyan_rebuild#(
    ...
    // 子模块例化 1（lpmmult_12_8）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 实例化 Line Buffer
    // 子模块例化 2（bilateral_filtering_Line_buffer_1）：为增强工程的双边滤波支路构造第二组对齐的 3×3 邻域窗口。
    ...
    // 子模块例化 3（shift_delay_256w_8d）：封装移位寄存器 IP，为像素、系数或同步信号提供固定拍数延迟。
    ...
    // 子模块例化 4（meiyan_rom_scurve）：封装只读存储器 IP，提供算法查找表或定点运算常量。
    ...
    // 双边滤波 Core
    // 子模块例化 5（meiyan_bilateral_core）：对肤色/亮度数据执行保边双边平滑，为磨皮融合提供去噪结果。
