...
// -----------------------------------------------------------------------------
// 正文导读：使用行存储构造连续 3×3 像素窗口，并将窗口数据与 HS/VS/DE 信号对齐。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：读写地址、突发长度、FIFO 清空和跨时钟握手必须保持一致，避免帧错位或数据溢出。
// -----------------------------------------------------------------------------
// 模块 bilateral_filtering_Line_buffer：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module bilateral_filtering_Line_buffer #(
    ...
    // LB2: 存中间行
    // 读写地址同步：当前列进，上一行同列出
    // 子模块例化 1（m4k_sync）：封装片上存储器 IP，为行缓存、帧内缓存或直方图统计提供存储资源。
    m4k_sync u_lb2 (
        .clock     (i_clk),
        .wren      (1'b1),               
        .wraddress (s_cnt),
    ...
    // LB1: 存最老行
    // 子模块例化 2（m4k_sync）：封装片上存储器 IP，为行缓存、帧内缓存或直方图统计提供存储资源。
    m4k_sync u_lb1 (
        .clock     (i_clk),
        .wren      (1'b1),              
        .wraddress (s_cnt_d1), 
    ...
    // 组合连线组 1：从 {o_p11,o_p12,o_p13} 开始的连续赋值随右值立即更新，不增加寄存器延迟。
