...
module top (
  ...
  // 组合连线组 1：从 CAM_XCLK 开始的连续赋值随右值立即更新，不增加寄存器延迟。
  ...
  // ---------- I2C 初始化 (SiI9134) ----------
  // 子模块例化 3（sii9134_i2c_init）：通过 I2C 初始化 SII9134 HDMI 发送器寄存器。
  sii9134_i2c_init #(
      .CLK_FRE(100)
  ) u_hdmi_i2c (
      .i_clk      (clk_sys),
  ...
  // 时序过程 2：由 clk_sys posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  always @(posedge clk_sys) begin
    if (sys_rst) begin
  ...
  reg [23:0] pclk_cnt = 0;
  // 时序过程 3：由 CAM_PCLK posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // ---------- I2C 初始化 (OV5640) ----------
  // 子模块例化 4（ov5640_i2c_init）：通过 I2C 寄存器序列初始化 OV5640 的分辨率、像素格式和输出时序。
  ov5640_i2c_init #(
      .CLK_FRE(100)
  ) u_cam_i2c (
      .i_clk      (clk_sys),
  ...
  // 组合连线组 1：从 led_cam_i2c_done 开始的连续赋值随右值立即更新，不增加寄存器延迟。
  ...
  // 子模块例化 5（ov5640_capture）：在像素时钟域采集 OV5640 RGB565 数据及同步信号，形成内部视频流。
  ...
  // 子模块例化 6（hdmi）：生成显示扫描时序，从帧缓存读取像素并形成 RGB888、HS、VS、DE 基准视频流。
  hdmi u_hdmi (
      .i_pclk       (clk_hdmi),
      .i_rst        (sys_rst),
  ...
  // 1. 例化 UART 接收模块
  // 子模块例化 7（uart_rx）：按设定波特率完成 UART 起始位检测、数据采样和字节有效指示。
  uart_rx #(
      .CLK_FRE(100),
      .BAUD_RATE(1000000),
      .DATA_WIDTH(8)
  ...
  // 子模块例化 8（uart_cmd_parser）：识别 0xAA/0x55 帧界定符，解析主模式与子模式并产生更新脉冲。
  ...
  // 子模块例化 9（cdc_handshake）：采用请求/应答握手在异步时钟域间可靠传递多位模式控制数据。
  cdc_handshake #(
  ...
  // 时序过程 4：由 clk_hdmi posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
  ...
  // 子模块例化 10（image_process_pipe）：实例化引导滤波/磨皮和暗光增强支路，根据主模式 10、11 选择最终输出视频流。
  ...
  // 子模块例化 11（sdram_top）：封装 SDRAM 控制器和读写 FIFO，提供视频帧缓存的统一顶层接口。
  ...
  // 组合连线组 1：从 hdmi_rgb888 开始的连续赋值随右值立即更新，不增加寄存器延迟。
  assign hdmi_rgb888 = rgb888_data;


  // 例化UART TX 模块
  // 子模块例化 12（uart_tx）：将并行字节按 UART 帧格式和设定波特率串行发送。
  uart_tx #(
      .CLK_FRE(100),
      .BAUD_RATE(1000000),
      .DATA_WIDTH(8)
