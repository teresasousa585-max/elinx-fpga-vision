...
`timescale 1 ns/ 1 ps
// -----------------------------------------------------------------------------
// 正文导读：集成灰度引导、局部统计、系数滤波和图像重建；子模式选择通用引导滤波或磨皮处理。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：像素数据、有效信号和 HS/VS 必须保持同拍；跨时钟数据必须使用 FIFO 或握手。
// -----------------------------------------------------------------------------
// 模块 guided_to_hdmi：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
...
// 子模块例化 2（guided_line_buffer）：构造引导图的局部窗口，输出均值与方差计算所需的邻域数据。
guided_line_buffer #(
   .H_TOTAL(H_TOTAL) 
)u_line_buffer (
    .i_clk(i_clk),
...
// 子模块例化 3（box_filter_y）：对引导图窗口执行盒式滤波，得到局部亮度均值及相关统计量。
...
// 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
wire [23:0] box_ycbcr;
// 组合连线组 1：从 box_ycbcr 开始的连续赋值随右值立即更新，不增加寄存器延迟。
...
// 子模块例化 4（guided_var_a_b）：根据局部统计量计算引导滤波线性系数 a、b。
guided_var_a_b #(
...
// 子模块例化 5（guided_line_buffer_a_b）：缓存系数 a、b 并构造其局部窗口，为系数均值计算提供数据。
...
// 子模块例化 6（box_filter_ab）：对线性系数 a、b 执行盒式均值滤波，并对齐中心像素旁路。
...
// 子模块例化 7（guided_final_rebuild）：使用均值系数和中心引导值重建滤波输出 q=mean(a)×I+mean(b)。
...
// 子模块例化 8（meiyan_rebuild）：融合原始像素与双边平滑结果，抑制皮肤纹理噪声并保留主要边缘。
...
always @(posedge i_clk) begin
    // 分支选择 1：依据 i_mode 选择状态或算法路径；default 覆盖非法或空闲条件。
    case(i_mode) 
        1'd0:begin
            hs_r <= m6_hs;
            vs_r <= m6_vs;
...
// 组合连线组 1：从 m8_hs 开始的连续赋值随右值立即更新，不增加寄存器延迟。
assign m8_hs = hs_r;
assign m8_vs = vs_r;
assign m8_de = de_r;
...
// 子模块例化 9（YCbCr_to_RGB）：将处理后的 YCbCr 像素恢复为 RGB888，并对齐原始旁路数据与视频同步信号。
