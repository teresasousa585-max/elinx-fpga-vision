...
module bilateral_core (
    ...
    // 子模块例化 1（rom_8_256）：封装只读存储器 IP，提供算法查找表或定点运算常量。
    ...
    always @(posedge i_clk) begin
        ...
        wt31 <= wr31 * W_SP_CORNER; wt32 <= wr32 * W_SP_EDGE; wt33 <= wr33 * W_SP_CORNER;
    ...
    always @(posedge i_clk) begin
        ...
        pwt31 <= y31_d4 * wt31; pwt32 <= y32_d4 * wt32; pwt33 <= y33_d4 * wt33;
    ...
    reg [17:0] inv_w;
    // T=7 送入地址，T=8 线出结果
    // 子模块例化 9（rom_reciprocal）：封装只读存储器 IP，提供算法查找表或定点运算常量。
    ...
    // 组合连线组 1：从 {o_hs, o_vs, o_data_en} 开始的连续赋值随右值立即更新，不增加寄存器延迟。
