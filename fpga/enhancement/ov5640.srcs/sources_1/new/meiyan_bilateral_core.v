...
`timescale 1 ns/ 1 ps
// -----------------------------------------------------------------------------
// 正文导读：对肤色/亮度数据执行保边双边平滑，为磨皮融合提供去噪结果。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：像素数据、有效信号和 HS/VS 必须保持同拍；跨时钟数据必须使用 FIFO 或握手。
// -----------------------------------------------------------------------------
// 模块 meiyan_bilateral_core：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module meiyan_bilateral_core #(
    ...
    // 子模块例化 1（shift_delay_32w_16d）：封装移位寄存器 IP，为像素、系数或同步信号提供固定拍数延迟。
    ...
    // 时序过程 8：由 i_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
    always @(posedge i_clk) begin
    ...
    // 时序过程 9：由 i_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
    always @(posedge i_clk) begin
        // T9
    ...
    wire [17:0] inv_w_w; 
    // 子模块例化 28（meiyan_rom_reciprocal）：封装只读存储器 IP，提供算法查找表或定点运算常量。
    meiyan_rom_reciprocal u_reciprocal (.address(rec_addr), .clock(i_clk), .q(inv_w_w));

    // 内部信号：用于流水级对齐、状态保存或子模块互连；位宽必须覆盖最坏计算范围。
    reg [27:0] p_sum_final_d1;
    // 时序过程 10：由 i_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
    always @(posedge i_clk) begin    
    ...
    // 子模块例化 29（lpmmult_28_18）：封装定点乘法器 IP，完成算法流水线中的乘法运算。
    ...
    // 时序过程 11：由 i_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
    always @(posedge i_clk) begin
    ...
    // 时序过程 12：由 i_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
    always @(posedge i_clk) begin
        y_out_d1 <= y_out;
        y_out_d2 <= y_out_d1;
    ...
    // 时序过程 13：由 i_clk posedge 触发，用于寄存数据、推进状态或对齐流水线；复位优先级不可随意调整。
    always @(posedge i_clk) begin
        if (i_rst) begin
            o_data <= 24'd0;
            {ohs, ovs, ode} <= 3'd0;
