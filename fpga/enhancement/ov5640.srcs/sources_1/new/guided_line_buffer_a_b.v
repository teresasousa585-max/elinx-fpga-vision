...
`timescale 1 ns/ 1 ps
// -----------------------------------------------------------------------------
// 正文导读：缓存系数 a、b 并构造其局部窗口，为系数均值计算提供数据。
// 阅读顺序：先确认参数和端口，再沿内部信号、时序过程及子模块例化追踪数据流。
// 修改约束：读写地址、突发长度、FIFO 清空和跨时钟握手必须保持一致，避免帧错位或数据溢出。
// -----------------------------------------------------------------------------
// 模块 guided_line_buffer_a_b：以下接口构成综合边界，上层通过端口连接数据流、控制流和状态信号。
module guided_line_buffer_a_b#(
    ...
    // 输出赋值
    // 组合连线组 1：从 o_col_sum_a 开始的连续赋值随右值立即更新，不增加寄存器延迟。
